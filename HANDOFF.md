...
- Fifth batch (tests 17–18, the first real full-match analysis — 5 hands against «خاطفين الولد»): **the partner led the suit → the Ace goes on his card** (the ban on giving an Ace to the partner's trick is about discarding it; measured neutral); and **the AI knows خاطفين الولد**: our side's ولد is never "the top trump" for pulling trumps, the habit isn't trusted over the search there (the playouts know the rule, play-ai doesn't), and the partner judges a hokum buy with his ولد counted as a 7 (`decideBid(..., weakJack)`). The solver showed that hand's pull with the weak ولد cost 138 raw points. Paired measurement under the rule is neutral within its noise (±0.3). Snapshots now carry `trickRules` so the analysis replays bent tricks right (older exports: the script maps the rival's name).
- **Analysing everything played:** every finished hand is saved in the browser across every صكة and run (`recordHand` in `src/game/notes.ts`, called by `GameController`; lean — ~3.4 KB a hand, the newest 400 kept). «ليش؟» has «📋 انسخ كل لعبك للتحليل — N يد» and a 🗑️ beside it to clear the record after sending. Claude saves the pasted text to a file and runs `npx vite-node scripts/analyze-match.ts <file> [minLoss=5]` — split big batches with `SHARD=1/4` … `SHARD=4/4` side by side. The double-dummy solver (`src/ai/solver.ts`, every hand face up, alpha-beta with a transposition table) replays each hand and lists every computer play that lost points against the best line, with what the AI was thinking, then a summary by decision kind and by the rules that were active. It's for reading patterns (a play the AI keeps getting wrong → a rule, a test, a paired measurement), not a verdict on single plays — the solver sees every card. Too slow for the phone (seconds to over a minute a hand), so it runs on Claude's side.
